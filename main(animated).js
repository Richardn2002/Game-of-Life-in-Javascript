...
function drawPoint(drawPosX,drawPosY,color){
    myCanvas.fillStyle=color;
    myCanvas.fillRect(1+(drawPosX-1)*pixelWidth,1+(drawPosY-1)*pixelWidth,pixelWidth,pixelWidth);
}
...
var interval = requestNumber("Interval?(ms)");
canvasInHTML.height=height*pixelWidth;
...
function once(){
    myCanvas.clearRect(1,1,width*pixelWidth,height*pixelWidth);
    for (var a=1;a<=width;a++){
        for(var b=1;b<=height;b++){
            dataNextTime[pos(a,b)] = liveOrNot(a,b);
            if(liveOrNot(a,b)){
                drawPoint(a,b,"000000");
            }
        }
    }
